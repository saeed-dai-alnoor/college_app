...
class TeacherUpdatingController extends GetxController {
  final GlobalKey<FormState> updatingTeacherFormKey = GlobalKey<FormState>();
  ...
  void checkTeacherUpdating(BuildContext context) async {
    CommonMethods.getConnectionType();
    if (!updatingTeacherFormKey.currentState!.validate()) {
      return;
    } else if (CommonMethods.connectionType == 0) {
      CommonMethods.showNoInternetDailog();
    } else {
      isLoading.value = true;
      final result = await TeacherProvider().updateTeacher(
      ...
      updatingTeacherFormKey.currentState!.save();
    }
  }
}
