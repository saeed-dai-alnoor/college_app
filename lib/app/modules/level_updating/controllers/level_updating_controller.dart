...
class LevelUpdatingController extends GetxController {
  final GlobalKey<FormState> updatingLevelFormKey = GlobalKey<FormState>();
  ...
  void checkLevelUpdating(BuildContext context) async {
    CommonMethods.getConnectionType();
    if (!updatingLevelFormKey.currentState!.validate()) {
      return;
    } else if (CommonMethods.connectionType == 0) {
      CommonMethods.showNoInternetDailog();
    } else {
      isLoading.value = true;
      final result = await LevelProvider().updateLevel(
      ...
      updatingLevelFormKey.currentState!.save();
    }
  }
}
